  ...
  return (
    <Slider {...settings}>
      {events.map((event, index) => (
        <Link href={`/events/${event.slug}`} key={index} className="p-4 transition-transform duration-300 hover:scale-110 hover:cursor-pointer">
          <div className="bg-customBlue text-white p-6 rounded-lg shadow-lg w-full h-56">
